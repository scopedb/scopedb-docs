...
If `<n>` is specified, MAX returns the N maximum values from a column, which is the same results as the regular query `FROM ... ORDER BY ... DESC LIMIT n` but as an array.
...
* If `<n>` is not specified, MAX returns the same as the data type of the input values.
* If `<n>` is specified, MAX returns an ARRAY that contains all the `<n>` maximum values.
