...
Use the `[]` operator for array indexing or object field access.
...
If the object does not contain the specified key, the function returns NULL.

String keys apply only to OBJECT values. To retrieve an ARRAY element, use an
integer index. Applying a string key to a non-object ANY value returns NULL.
