...
* `POINT`: An index that optimizes performance of predicate forms based on `=`, `IS` and `contains(...)`. On `object` and `any` expressions, a point index can also prune predicates on nested paths inside the value.
...
1. Use `POINT` or `RANGE` indexes when:
   ...
   - You filter on nested paths inside an `object` or `any` column.
