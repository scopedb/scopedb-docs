...
* Between a value in ANY type and another data type. If the value is not
  compatible with the target data type, the cast raises an error. Wrap the cast
  in [`TRY`](/reference/functions/conversion#try-cast) to return NULL instead.
