...
To convert a value to or from the variant data type, you can explicitly cast using the CAST function or the `::` operator (e.g. `<expr>::variant`). For details, see [data type conversion](datatypes-conversion.mdx).
...
See also [variant data functions](functions-variant.mdx).
