...
relatedContents:
    - title: Storage architecture
      url: /guides/storage-architecture
...
Console, SDKs, the ScopeDB CLI, and the HTTP data API.
...
| ScopeDB API address | The HTTPS base URL shown under its purpose label in **Connect**. SDKs, HTTP clients, and non-interactive CLI jobs use it as their endpoint. |
| API key | A secret credential for SDKs, HTTP clients, and non-interactive CLI jobs. Send it as a bearer token and keep it out of client-side applications. |
...
your workspace. For interactive CLI use, sign in with your ScopeDB Cloud
account instead of configuring an API key.
...
SDK and HTTP steps, or use the [ScopeDB CLI](/developer/scopedb-cli) to sign in
interactively.
