...
```scopeql
CREATE RANGE INDEX ON events (time);
CREATE POINT INDEX ON events (service);
CREATE POINT INDEX ON events (var);
```
...
```scopeql
CREATE POINT INDEX ON events (var['gateway']::string);
```

For text investigation, use a search index on the expression users search:

```scopeql
CREATE SEARCH INDEX ON events (message);
```
...
```scopeql
CREATE MATERIALIZED INDEX ON events (var['http']['url']::string);
```
