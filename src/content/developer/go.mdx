...
relatedContents:
    ...
    - title: Go package reference
      url: https://pkg.go.dev/github.com/scopedb/goscopedb
    - title: Go SDK repository
      url: https://github.com/scopedb/goscopedb
---
...
Install the maintained Go SDK module from your application module, then commit
the selected version in `go.mod` and `go.sum`:
...
`AppendStream` is the simpler path when the SDK should own encoding and request
boundaries.
...
- Browse the [Go SDK examples](https://github.com/scopedb/goscopedb/tree/main/examples)
